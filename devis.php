...
{
    echo '<ul class="flex flex-wrap gap-4">';
...
<main class="flex flex-col flex-grow bg-gray-50 dark:bg-gray-900 transition-colors duration-500">
        ...
        <div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 shadow-lg rounded-lg overflow-hidden transition-colors duration-500">
            ...
            <div class="px-6 pb-12">
                <?php if ($recaptchaConsent === 'true'): ?>
                    <form id="devisForm" data-recaptcha="true" novalidate class="space-y-6">
                        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                        <input type="hidden" name="formType" value="devis">
                        <input type="hidden" name="recaptcha_token" id="recaptcha_token">

                        <div class="p-6 bg-gray-50 dark:bg-gray-900 rounded-xl shadow-inner space-y-4 fly-yoyo">
                        ...
                        <div class="p-6 bg-gray-50 dark:bg-gray-900 rounded-xl shadow-inner space-y-4 max-w-[250px] fly-yoyo">
                        ...
                        <div class="p-6 bg-gray-50 dark:bg-gray-900 rounded-xl shadow-inner space-y-6 fly-yoyo">
                        ...
                        <div class="p-6 bg-gray-50 dark:bg-gray-900 rounded-xl shadow-inner space-y-4 fly-yoyo">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 items-start">
                                <?php
                                $services = [
                                    ["Office 2024 installé", 29, ""],
                                    ["Suite Adobe 1 an", 150, ""],
                                    ["Optimisation gaming", 29, "Profil XMP ou EXPO activé, configuration Windows 10 pour le gaming (réglages du système et optimisation des performances), optimisation du panneau de configuration de votre carte graphique (NVIDIA/AMD). Installation de tous les drivers pour votre processeur, carte graphique, et SSD."],
                                    ["Overclock GPU", 40, "Augmentation de la vitesse d'horloge de votre carte graphique pour des meilleures performances en jeu."],
                                    ["Undervolting GPU", 40, "Réduction de la tension de votre carte graphique pour diminuer sa consommation électrique et sa chaleur, améliorant ainsi la longévité et le silence de votre PC, sans perte de performance (Souvent combiné avec l'overclocking GPU)."],
                                    ["Dual-boot Linux/Windows", 60, "Démarrer votre PC en choisissant entre Windows et Linux ! Profitez de la puissance de deux systèmes d'exploitation sur la même machine pour une flexibilité maximale."],
                                ];
                                foreach ($services as [$label, $price, $descriptionText]):
                                    $id = preg_replace('/[^a-z0-9]/i', '_', strtolower($label));
                                    $showDescription = !empty($descriptionText);
                                ?>
                                    <div class="flex flex-col p-3 rounded-xl border border-gray-200 dark:border-gray-700 hover:shadow-md transition fly-yoyo">
                                        <label class="flex justify-between items-center w-full cursor-pointer">
                                            <span><?= $label ?></span>
                                            <div class="flex items-center gap-2">
                                                <span class="text-gray-500 dark:text-gray-400"><?= $price ?> €</span>
                                                <input type="checkbox" name="services[]" id="<?= $id ?>" value="<?= htmlspecialchars($label . ' (' . $price . ' €)') ?>" class="form-checkbox h-6 w-6 text-green-400 border-gray-300 dark:border-gray-600">
                                            </div>
                                        </label>
                                        <?php if ($showDescription): ?>
                                            <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                                <div class="description-container">
                                                    <p class="pt-2"><?= $descriptionText ?></p>
                                                </div>
                                                <button type="button" onclick="toggleDescription(this)" class="mt-1 text-blue-500 hover:underline">En savoir plus</button>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                <?php endforeach; ?>
                    ...
                    <script>
                        ...
                        function toggleDescription(button) {
                            // On remonte jusqu'à la carte (le conteneur principal de ce service)
                            const card = button.closest('.flex.flex-col');
                            const descElement = card.querySelector('.description-container');

                            descElement.classList.toggle('show');

                            button.textContent = descElement.classList.contains('show') ?
                                'Moins d\'infos' :
                                'En savoir plus';
                        }
